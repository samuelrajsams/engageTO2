(function (angular) {
    ...
    angular
            .module('landing')
            .directive('changePasswordCheckerDirective', changePasswordCheckerDirective);

    function changePasswordCheckerDirective() {
        var changePasswordCheckerDirective = {
            restrict: 'EA',
            replace: false,
        ...
        return changePasswordCheckerDirective;
    ...
    function passwordCheckerLink(scope, iElement, iAttrs) {
        ...
        scope.$watch(iAttrs.changePasswordCheckerDirective, function () {
            if (scope.changePassword.newPassword === '' || !scope.changePassword.newPassword) {
                iElement.css({"display": "none"});
            } else {
                iElement.css({"display": "inline"});
                var c = strength.getColor(strength.mesureStrength(scope.changePassword.newPassword));
                scope.changePassword.validPassword = c.idx >= 5 ? true : false;
                if (c.idx <= 1) {
