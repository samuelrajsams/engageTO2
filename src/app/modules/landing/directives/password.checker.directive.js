(function (angular) {
    ...
    angular
            .module('landing')
            .directive('passwordCheckerDirective', passwordCheckerDirective);

    function passwordCheckerDirective() {
        var passwordCheckerDirective = {
            restrict: 'EA',
            replace: false,
        ...
        return passwordCheckerDirective;
...
//                _force = (_passedMatches == 3 && _force > 10) ? Math.min(_force, 50) : _force;
        ...
        scope.$watch(iAttrs.passwordCheckerDirective, function () {
            if (scope.resetPassword.password === '' || !scope.resetPassword.password) {
                iElement.css({"display": "none"});
            } else {
                iElement.css({"display": "inline"});
                var c = strength.getColor(strength.mesureStrength(scope.resetPassword.password));
                scope.resetPassword.validPassword = c.idx >= 5 ? true : false;
                if (c.idx <= 1) {
